...
function EventTile({ event, attending, hosting }) {
...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8000/profile?email=${encodeURIComponent(session.email)}`)
      ...
      .then((data) => {
        if (data.success) {
          setUserEventType(data.event_type || '');
            ...
            {hasAiInterests && loadingSuggestedEvents ? (
              ...
              <div className="events-grid">
                {(!hasAiInterests || !loadingSuggestedEvents) && displayedSuggestedEvents.map((event) => (
                  <EventTile key={event.id} event={event} attending={attendingEventIdSet.has(event.id)} hosting={event.owner_email === session.email} />
                ))}
              </div>
            )}
          </section>

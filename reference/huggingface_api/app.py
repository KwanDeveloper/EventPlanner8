...
classification_model = ORTModelForSequenceClassification.from_pretrained(CLASSIFICATION_MODEL_REPO, provider="CPUExecutionProvider", session_options=session_options)
...
classification_model = ORTModelForSequenceClassification.from_pretrained(CLASSIFICATION_DIR, provider="CPUExecutionProvider", session_options=session_options)
...
sentence_model = ORTModelForFeatureExtraction.from_pretrained(SENTENCE_MODEL_REPO, provider="CPUExecutionProvider", session_options=session_options)
...
sentence_model = ORTModelForFeatureExtraction.from_pretrained(SENTENCE_DIR, provider="CPUExecutionProvider", session_options=session_options)
...
def generate_classification(input_text: str, choices: list[str], normalize: bool):
...
def generate_sentence(input_text: str, choices: list[str], normalize: bool):
...
def generate(input_json: str):
    ...
    output_data = {"id": task_id, "task": task, "output": output, "highest": output[0] if output else None, "lowest": output[-1] if output else None, "generation_time": generation_time}
